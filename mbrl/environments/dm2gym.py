...
try:
    ...
    from dm_control.viewer import runtime, util
...
class DmControlWrapper(GroundTruthSupportEnv, ABC):
    ...
    def step(self, a):
        if isinstance(self.action_space, DmcDiscrete):
            a += self.action_space.offset
        reward = 0
        for _ in range(self._action_repeat):
            timestep = self.dmcenv.step(a)
            reward += timestep.reward or 0
        self.timestep = timestep
        return self._get_observation(), reward, self.timestep.last(), {}
