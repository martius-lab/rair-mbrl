...
class ShapeGridworld(gym.Env):
    ...
    def __init__(
        ...
        size_color_base_vecs = int(np.ceil(np.log2(self.num_colors)))
        multiply_color_factor = int(np.ceil(self.num_objects / self.num_colors))

        combinations = np.asarray(list(product([0, 1], repeat=size_color_base_vecs)))
        # Shape of combinations: [2^num-size_color_base_vecs, size_color_base_vecs]

        self.object_colors = np.tile(
            combinations[: self.num_colors, :], (multiply_color_factor, 1)
        )[: self.num_objects]
        self.color_feature_size = size_color_base_vecs
        self.colors = (colors[: self.num_colors] * multiply_color_factor)[
            : self.num_objects
        ]

        # print("self.object colors: i.e features for color appended to object: ", self.object_colors)
        # print("self.colors: i.e actual object color as rendered: ", self.colors)

        # self.colors = utils.get_colors(num_colors=max(9, self.num_objects), cmap="RdPu")
        # self.object_colors = np.round(np.asarray(self.colors)[: self.num_objects, :3], decimals=1)
        self.np_random = None
        self.game = None
        self.target = None
        ...
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(self.num_objects * (2 + self.color_feature_size) + 2,),
            dtype=np.float32,
        )
