...
class RND(nn.Module):
    def __init__(self, model_params, clip_val=5.0):
        ...
        if self.rnd_network_type == "mlp":
            ...
            if self.obs_dim != self.agent_dim + self.nObj * (
                self.object_dyn_dim + self.object_stat_dim
            ):
...
class TorchRNDMpcICem(TrainableController, TorchMpcICem):
    ...
    def train_rnd(self, rollout_buffer: RolloutBuffer):
        ...
        if self.epochs:
            ...
            epoch_length = np.ceil(
                iterator_train.array["inputs"].shape[0] / self.batch_size
            )
        elif self.iterations:
            iterator = iterator_train.get_basic_iterator(
                self.batch_size, self.iterations
            )
            epoch_length = 1
    ...
    def _model_epistemic_costs(self, rollout_buffer: RolloutBuffer):
        ...
        # or # shape: [p,e,h,obs_dim] with ensemble size!
        if self._ensemble_size:
            ensemble_dim = 1
            mean_next_obs = torch.mean(mean_next_obs, dim=ensemble_dim)

        rnds_of_samples_ = self.compute_intr_reward(
            mean_next_obs.reshape(-1, self.env.observation_space.shape[0])
        )
    ...
    def trajectory_cost_fn(
        ...
        if self._ensemble_size:
            costs_path = (
                -self._rnd_bonus_per_path.unsqueeze(1)
                .expand(-1, self._ensemble_size, -1)
                .clone()
            )
        else:
            costs_path = -self._rnd_bonus_per_path.clone()
