...
def get_values2compress_jit(
    ...
    diff_matrix = granularity * np.floor(diff_matrix * precision / granularity)

    return diff_matrix[:, :, mask, :]
...
def get_values2compress_torch(
    ...
    diff_matrix = granularity * torch.floor(diff_matrix * precision / granularity)

    return diff_matrix[:, :, mask, :]


#  iCEM with realtional RaIR with (absolute) difference (absolute -> bidirectional=False)
class TorchRelationalRairMpcICem(TorchMpcICem):
    ...
    def preallocate_memory(self):
        ...
        if self.bidirectional:
            self.mask[range(0, self.env.nObj**2, self.env.nObj + 1)] = False
        else:
    ...
    def _model_rair_costs(self, rollout_buffer: RolloutBuffer):
        ...
        if self._ensemble_size:
            ...
            # Now back to object centric view!
        ...
        if torch_helpers.device == "cpu" or self.mode == "normal":
            ...
            values2compress = get_values2compress_jit(
                next_obs_ensemble_mean,
            ...
            # Values2compress shape: [num_particles, horizon, num_relation_matrix_entities, compression_ndim]
